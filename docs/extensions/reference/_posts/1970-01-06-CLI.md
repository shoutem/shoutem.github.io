...
Executes packager locally to **run** and **debug** custom or existing extensions using **Shoutem Preview** app (available for [iOS]({{ site.shoutem.previewAppiOS }}) and [Android]({{ site.shoutem.previewAppAndroid }})) on both Android and iOS without having to setup Android Studio and/or XCode. Moreover, you can use Windows to develop iOS apps. The only 2 things you would need to run the app are `react-native-cli` and `Node.js` v7. See our [FAQ]({{ site.url }}/docs/extensions/tutorials/faq) if you have problems setting this up.
