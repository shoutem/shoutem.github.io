...
{{ site.example.devName }}.{{ ste.example.extensionName }}/
  ...
  └ extension.json
```
Please note that {{ site.example.devName }} and {{ ste.example.extensionName }} are just used as an example.
...
Runs platform's configure script to sync the local app with native changes to local extensions.
