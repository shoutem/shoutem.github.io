...
In the example above, we included 2 screen objects inside of the `screens` array. We wanted to show you the usage of `extends` field. Extending makes it possible to [switch between multiple alternative layouts](/docs/extensions/tutorials/alternative-layouts).
